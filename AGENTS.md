# Single-binary package registry proxy that enforces dependency cooldowns at the network layer (`cooldown-registry-proxy`) — factory product contract

This repository is one product built by the Param Factory. It ships at
`https://cooldown-registry-proxy.sociobot.in`. Artifact class: `cli`.
